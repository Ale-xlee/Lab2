...
_What are the variable data types?_ characters and integers
...
**NOt applicable, summing integers and NAs doesnt work, so using na.rm = TRUE to filter them out**
...
**na.rm = TRUE removes NA inputs from calculation**
...
**Skewed by large numbers of visitors and a smaller number of venues**
...
```{r Ex_m}
free_entry<-visitors_with_nations %>%
  group_by(nation) %>%
  count(admission) %>%
    # mutate( prop_free = )
...
free_entry
# 
# total_count <- visitors_with_nations %>%
#   group_by(nation) %>%
#   count()
# 
# prop_free <- free_entry / total_count 
